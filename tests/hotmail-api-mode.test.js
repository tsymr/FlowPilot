...
test('Hotmail API对接界面文案应启用 remote 模式并隐藏禁用态', () => {
  ...
  assert.match(
    html,
    /<span class="data-label">邮件API<\/span>/,
    'Hotmail 配置行标签应为邮件API'
  );
});

test('Hotmail API对接应接入邮件 API 服务', () => {
  const background = fs.readFileSync('background.js', 'utf8');

  assert.match(background, /importScripts\([^)]*microsoft-email\.js[^)]*\)/, 'background 应加载微软邮箱 helper');
  assert.match(background, /buildEmailApiListUrl/, '账号校验和最新验证码应使用邮件 API 列表接口');
  assert.match(
    background,
